...
{
	private HashSet<ElementReference>? _excludedElements;
	...
	public async ValueTask DisposeAsync()
	{
		...
		if (_underlyingPanzoomInterop != null) await _underlyingPanzoomInterop.DisposeAsync();
	}
	...
	protected override async Task OnAfterRenderAsync(bool firstRender)
	{
		if (firstRender)
		{
			if (ElementReference.IsDefault()) throw new ArgumentException("ElementReference must be set for Panzoom to work");
			...
			{
				if (!OnWheel.HasDelegate) throw new ArgumentException("OnWheel must be set when using WheelMode.Custom!");
			...
			if (SetTransform.HasDelegate) await PanzoomHelper.SetTransformAsync(_underlyingPanzoomInterop, SetTransform);
	...
	{
		if (_excludedElements is null) return;

		if (_excludedElements.Count == 0) return;
