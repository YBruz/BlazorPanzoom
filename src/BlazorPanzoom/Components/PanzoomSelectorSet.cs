...
public class PanzoomSelectorSet : ComponentBase, IAsyncDisposable
{
	...
	protected override async Task OnAfterRenderAsync(bool firstRender)
	{
		if (firstRender)
		{
			PanzoomInterop[] list = await PanzoomHelper.CreateForSelectorAsync(Selector, PanzoomOptions);

			foreach (var panzoom in list) _panzoomSet.Add(panzoom);

			if (OnAfterCreate.HasDelegate) await OnAfterCreate.InvokeAsync();
		}
